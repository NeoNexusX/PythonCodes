            ...
            for idxc in range(self.channel_out):
                for idxh in range(height_out):
                    for idxw in range(width_out):
                        self.output[idxn, idxc, idxh, idxw] = np.sum(self.weight[:, :, :, idxc] * self.input_pad[
                                                                                                  idxn,
                                                                                                  :,
            ...
            for idxc in range(top_diff.shape[1]):
                for idxh in range(top_diff.shape[2]):
                    for idxw in range(top_diff.shape[3]):
                        ...
                        # print(top_diff[idxn, idxc, idxh, idxw].shape)
                        # print(self.input_pad[ :,:,self.stride * idxh:self.stride * idxh + self.kernel_size,self.stride * idxw:self.stride * idxw + self.kernel_size].shape)
                        self.d_weight[:, :, :, idxc] += top_diff[idxn, idxc, idxh, idxw] * self.input_pad[
                                                                                        idxn,
                                                                                        :,
                                                                                        self.stride * idxh:self.stride * idxh + self.kernel_size,
                                                                                        self.stride * idxw:self.stride * idxw + self.kernel_size]
                        ...
                        :,
                        idxh * self.stride:idxh * self.stride + self.kernel_size,
                        idxw * self.stride:idxw * self.stride + self.kernel_size] += top_diff[idxn, idxc, idxh, idxw] * self.weight[:, :, :, idxc]
            ...
            for idxc in range(self.input.shape[1]):
                for idxh in range(height_out):
                    for idxw in range(width_out):
                        ...
                        self.output[idxn, idxc, idxh, idxw] = np.max(self.input[
                                                                     idxn,
                                                                     idxc,
                                                                     idxh * self.stride:idxh * self.stride + self.kernel_size,
                                                                     idxw * self.stride:idxw * self.stride + self.kernel_size])
                        ...
                        self.max_index[
                            idxn, idxc, idxh * self.stride + curren_max_index[0], idxw * self.stride + curren_max_index[1]] = 1
        return self.output
            ...
            for idxc in range(top_diff.shape[1]):
                for idxh in range(top_diff.shape[2]):
                    for idxw in range(top_diff.shape[3]):
                        ...
                        bottom_diff[idxn, idxc, idxh * self.stride + max_index[0], idxw * self.stride + max_index[1]] = top_diff[idxn, idxc , idxh, idxc]
        return bottom_diff
