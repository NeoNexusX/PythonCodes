...
class VGG19(object):
    ...
    def backward(self, dloss, layer_name):
        ...
        for idx in range(layer_idx, -1, -1):
            # TODO： 计算VGG19网络的反向传播
            dloss = self.param_layer_name[layer_idx[idx]].backward(dloss)
...
class AdamOptimizer(object):
    ...
    def update(self, input, grad):
        ...
        output = input - input * (self.lr*self.mt / np.sqrt(vt_hat)+ 1e-8)
        return output
...
if __name__ == '__main__':
    ...
    for step in range(TRAIN_STEP):
        ...
        for layer in CONTENT_LOSS_LAYERS:
            ...
            content_diff += dloss#vgg.backward(dloss,layer)
        for layer in STYLE_LOSS_LAYERS:
            ...
            style_diff += dloss #vgg.backward(dloss,layer)
